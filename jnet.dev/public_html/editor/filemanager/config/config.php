...
if(isset($_COOKIE['jnet_session']))
		$ci_session = $_COOKIE['jnet_session'];
	else 
		$ci_session = "not";
	$db = new database();
	 $db->query("select * from log_session where ci_session = '{$ci_session}' limit 1");
	$user = $db->fetch();
	if(!isset($user['username']))
		die("Bạn không có quyền upload hình ảnh, vui lòng liên hệ JNET để được hỗ trợ.");
	else 
		$user  = $user['username'];	
/*
|--------------------------------------------------------------------------
...
$config = array(
	...
	*/
	...
	|
	*/
	'upload_dir' => '/uploads/'.$user.'/images/',
	...
	|
	*/
	'current_path' => '../../uploads/'.$user.'/images/',
	...
	|
	*/
	'thumbs_base_path' => '../../uploads/'.$user.'/thumbs/',
	...
	*/
	...
	//**********************
	'ext_img'                                 => array( 'jpg', 'jpeg', 'png', 'gif', 'JPG','PNG' ), //Images
