...
if ( ! empty($_FILES))
{
	...
	if (in_array(fix_strtolower($info['extension']), $ext))
	{
		...
		$_FILES['file']['name'] = fix_filename($_FILES['file']['name'],true,true, "_");
		...
		$targetFile =  $targetPath.$key.$_FILES['file']['name'];
		
		
	
		
		$targetFileThumb =  $targetPathThumb. $key .$_FILES['file']['name'];
		
		//thêm size mới
		$targetFileThumb_90 =  $targetPathThumb. "90x90_".$key.$_FILES['file']['name'];
		$targetFileThumb_300 =  $targetPathThumb. "300x300_".$key.$_FILES['file']['name'];	
		...
		if ($is_img)
		{
			...
			// hàm tạo size mới 90x90
			create_img($targetFile, $targetFileThumb_90, 90, 90);
			// hàm tạo size mới 300x300
			create_img($targetFile, $targetFileThumb_300, 300, 300);
			// kết thúc
			if ( ! create_img($targetFile, $targetFileThumb, 242, 243))
			{
				$memory_error = FALSE;
			}
			else
