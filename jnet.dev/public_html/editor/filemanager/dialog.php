...
elseif($type_param==2) $apply = 'apply_img';
elseif($type_param==0 && $_GET['field_id']=='') $apply = 'apply_none';
elseif($type_param==3) $apply = 'apply_img';
else $apply = 'apply_img';
...
<html xmlns="https://www.w3.org/1999/xhtml">
	<head>
		...
		<link rel="shortcut icon" href="https://jnet.vn/template/frontend-admin/1/images/favicon.png">
		...
		<?php
			...
			onReady: function() {
	      	...
	      	onSave: function(imageID, newURL) {
				...
				$.ajax({
					type: "POST",
					url: "ajax_calls.aspx?action=save_img",
...
<body>
	...
	<input type="hidden" id="new_folder" value="<?php echo trans('New_Folder'); ?> (không dấu và không có khoảng cách)" />
	...
	<div class="tabbable upload-tabbable"> <!-- Only required for left/right tabs -->
	    ...
	    <div class="tab-content">
			<div class="tab-pane active" id="tab1">
		    	<?php } ?>
				<form action="popup.aspx" method="post" enctype="multipart/form-data" id="rfmDropzone" class="dropzone">
...
<!-- header div end -->
	...
	<li class="pull-right"><a id="refresh" class="btn-small" href="popup.aspx?<?php echo $get_params.$subdir."&".uniqid() ?>"><i class="icon-refresh"></i></a></li>
	...
	<div class="span12">
	    ...
	    <br/>
	    <div class="alert alert-error">Bạn không có quyền upload ảnh. Vui lòng liên hệ JNET để được hỗ trợ. </div>
	    <?php }else{ ?>
		...
		foreach ($files as $file_array) {
		  $file=$file_array['file'];
			...
			?>
			    ...
			    <?php } ?>
				  <a class="folder-link" href="popup.aspx?<?php echo $get_params.rawurlencode($src)."&".uniqid() ?>">
			...
			<?php }else{ ?>
				    ...
				    <div class="box">
					<h4 class="<?php if($ellipsis_title_after_first_row){ echo "ellipsis"; } ?>"><a class="folder-link" data-file="<?php echo $file ?>" href="popup.aspx?<?php echo $get_params.rawurlencode($src)."&".uniqid() ?>"><?php echo $file; ?></a></h4>
				    </div>
...
?>
            ...
            ?>		<figure data-name="<?php echo $file ?>" data-type="<?php if($is_img){ echo "img"; }else{ echo "file"; } ?>">
				...
				<figcaption>
				    <form action="force_download.aspx" method="post" class="download-form" id="form<?php echo $nu; ?>">
					...
					title="<?php echo trans('Preview')?>" data-url="ajax_calls.aspx?action=media_preview&title=<?php echo $filename; ?>&file=<?php echo $rfm_subfolder.$subdir.$file; ?>"
					href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
					    ...
					    <a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="ajax_calls.aspx?action=get_file&sub_action=preview&preview_mode=text&title=<?php echo $filename; ?>&file=<?php echo $rfm_subfolder.$subdir.$file; ?>"
						href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
						<?php }elseif($googledoc_enabled && in_array($extension_lower,$googledoc_file_exts)){ ?>
					    <a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="ajax_calls.aspx?action=get_file&sub_action=preview&preview_mode=google&title=<?php echo $filename; ?>&file=<?php echo $rfm_subfolder.$subdir.$file; ?>"
						href="docs.google.com;" ><i class=" icon-eye-open"></i></a>

						<?php }elseif($viewerjs_enabled && in_array($extension_lower,$viewerjs_file_exts)){ ?>
					    <a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="ajax_calls.aspx?action=get_file&sub_action=preview&preview_mode=viewerjs&title=<?php echo $filename; ?>&file=<?php echo $rfm_subfolder.$subdir.$file; ?>"
						href="docs.google.com;" ><i class=" icon-eye-open"></i></a>
