...
<!--
	...
	function changeTheme(idtheme) {
		...
		$.post("<?php echo $user_session['url'] ?>/dashboard/GetlistTemplate/change_theme", {
			idStore : <?php echo $user_session['id_store'] ?>,
			idTheme : idtheme
		}, function(data) {
				if(data == 1) {
					$("span#btn-active").html('Đã kích hoạt');
					$("#modal-content"+idtheme).animate({
			           height: '200px',
			     	});
					$("#body"+idtheme).html('<div class="row text-center"><h3>Kích hoạt giao diện thành công.</h3></div>');
					$("#modal-footer"+idtheme).html('<a class="btn btn-primary" href="<?php echo $user_session['url'] ?>/dashboard/template/customize.html">Đi đến tùy chỉnh giao diện</a> <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>');
				} else {
					$("span#btn-active").html('Kích hoạt');
					alert("Lỗi !");
				}	
		  ...
		  <div class="modal fade" id="themeModal<?php echo $tem->id_theme ?>" role="dialog">
		    <div class="modal-dialog modal-lg">
		      ...
		      <div class="modal-content" id="modal-content<?php echo $tem->id_theme ?>">
		        ...
		        <div class="modal-footer" id="modal-footer<?php echo $tem->id_theme ?>" style="text-align: center">
