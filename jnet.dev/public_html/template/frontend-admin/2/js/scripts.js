// on document ready
	...
	window.onload = function() {
	 var myInput = document.getElementById('ReEmail');
	...
	function validation_form() {
		...
		if($('#ReEmail').val() == '' || isEmail($('#ReEmail').val()) == false || $('#ReEmail').val() != $('#Email').val()) {
		...
		$('.btn_create').html('<i class="fa fa-cog fa-spin fa-3x fa-fw margin-bottom"></i>');
		$('.btn_create').prop('disabled', true);
			...
			$.post("store/register", {
				StoreName : remove_unicode(StoreName),
...
(function($){
	...
	(function(){
		...
		if(windowWidth <= 630) {
			...
			$('.menu_desktop').css('display','block');
		}	
		...
		window.sticky = function(){
			...
			container.waypoint(function(direction){
				...
				if(direction == "down"){
					...
					menu.addClass('small_header');
					//$('.menu_wrap hr').css('margin-top','0px');
					$('.menu_wrap hr').css('display','none');
					//container.parent('[role="banner"]').css('border-bottom',mHeight + "px solid transparent");
					
				}else if(direction == "up"){
					menu.removeClass('sticky');
					menu.removeClass('small_header');
					container.parent('[role="banner"]').css('border-bottom','none');
					$('.menu_wrap hr').css('display','block');
